...
class LoveWebsite {
    constructor() {
        ...
        this.photoIndex = 0;
        this.photos = ['P2.jpeg', 'P3.jpeg', 'P4.jpeg'];
    ...
    initializeMusic() {
        ...
        // Start muted to satisfy autoplay policies, then unmute shortly
        ...
        tryPlay();
        // Unmute after a brief delay and retry a few times
        setTimeout(() => {
            if (!this.music) return;
            // Smooth fade-in from 0 to 1 volume
            this.music.muted = false;
            this.music.volume = 0.0;
            const fade = setInterval(() => {
                if (!this.music) { clearInterval(fade); return; }
                if (this.music.volume < 1.0) {
                    this.music.volume = Math.min(1.0, this.music.volume + 0.1);
                } else {
                    clearInterval(fade);
                }
            }, 150);
            // Retry play a few times in case it paused
            let attempts = 0;
            const interval = setInterval(() => {
                if (!this.music) { clearInterval(interval); return; }
                if (!this.music.paused) { clearInterval(interval); return; }
    ...
    createConfetti() {
        // Navratri-themed palette: saffron, white, green, marigold, deep red
        const colors = ['#ff9933', '#ffffff', '#138808', '#ffd166', '#c0392b'];
...
const romanticQuotes = [
    "Happy Navratri! May Maa Durga fill your life with shakti ✨",
    "Navratri cha har ek divas tuzya hasnyane ujvale 💛",
    "May your 9 days be full of new rang and new beginnings 🌼",
    "Garba of joy, dandiya of dreams, and dhun of happiness 🎶",
    "Tuzya jivanat darroj navin aasha phulate raho 🌸",
    "Blessings, courage, and khushiyaan—this Navratri and always 🙏",
    "Har pal me rang, har din me umed—Happy Navratri! 🌈",
    "Smile wide, dance free, celebrate beautifully 💃",
    "Maa Durga tuzya har ichhe la shakti devu de ❤️",
    "Ya varsha chi Navratri saglyat special thavu de ✨"
];

function showRandomQuote(force = false) {
    if (force || Math.random() < 0.35) { // show if forced or by chance
        ...
        quoteElement.style.cssText = `
            ...
            z-index: 2002; /* above nav dots and confetti */
...
// Force one after 4 seconds, then occasionally
setTimeout(() => showRandomQuote(true), 4000);
setInterval(showRandomQuote, 12000); // every 12 seconds
